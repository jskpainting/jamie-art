...
| 2026-06-12 | Smoke-test fix: `AspectGrid` switched from `h-full w-full` to `w-full h-auto` on `<Image>` | Next.js Image (non-fill mode) injects `height: auto` as an inline style that overrides the `h-full` Tailwind class, collapsing images inside aspect-ratio containers. Using `w-full h-auto` delegates sizing to Next.js and the image's intrinsic dimensions. `loading="eager"` added to both `AspectGrid` and `MasonryGrid` so images aren't lazy-skipped while the AnimatePresence enter animation runs at opacity 0. |
...
| 2026-08-19 | Owner-applied brightness/contrast (IE-3) is deliberate and distinct from "the site never filters `<img>`" | §4.1/§11 already establishes that painting color fidelity is preserved on the *rendered* site — no CSS filters, no overlays. IE-3 lets the *owner* nudge exposure on a photo they're uploading, baked into the file at save time via `ctx.filter` in `lib/image-edit.ts#renderEdit`, not applied live by any public page. Guardrails keep it conservative: brightness 70–130%, contrast 85–115% only (no hue/saturation/warmth/rotation), a touch-safe Compare (hold to see the untouched photo) and Reset, and non-destructive storage (`image_edits` table) so "Revert to original" is always available. Painting presets show explicit copy under the sliders reinforcing that colours should still look true. |
